...
export function CsaDrawer({ csa, activeParty }: Props) {
  ...
  // Pre-fill the Post Collateral modal with the outstanding call ONLY
  // when it's directed at the active party (the funder side). Posting
  // from the non-funder side moves csb in the wrong direction — each
  // wrong-side post DOUBLES the gap because PostCollateral always
  // increments the poster's signed contribution. The non-funder still
  // gets a counterparty-warning banner in the modal so they understand
  // the call isn't theirs to clear.
  const callSignal = latest != null ? computeCallSignal(csa, latest.exposure) : null
  const viewerIsFunder =
    callSignal !== null && ((isA && callSignal.side === 'A') || (!isA && callSignal.side === 'B'))
  const outstandingCallAmount = viewerIsFunder ? callSignal.amount : null
  const counterpartyHasOpenCall = callSignal !== null && !viewerIsFunder
          ...
          {isOperator ? (
            ...
            <>
              <CsaFundingActions
                ...
                counterpartyHasOpenCall={counterpartyHasOpenCall}
              />
